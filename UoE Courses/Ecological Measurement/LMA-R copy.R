...
#MORE PLOTS
LMA_table <- LMA_raw_data  %>% group_by(TID) %>% summarise (mean_LMA =  mean(MLMA), 
                                                            stdev = sd(MLMA) ,
                                                            se_LMA = sd(MLMA)/sqrt(n()))

lma <- ggplot(LMA_raw_data) + geom_point(aes(TID, MLMA, col=TID), shape=21)

lma <- lma + geom_point(data=LMA_raw_data, aes(TID, MLMA,col = TID),
